...
impl QueryMetadata {
    pub fn get_cigar_ops(
        ...
        if ["gz", "bgz"]
            .iter()
            .any(|e| paf_file.extension() == Some(e.as_ref()))
        {
        ...
        parse_cigar_to_delta(cigar_str)
            .ok()
...
impl Impg {
    pub fn from_paf_records(records: &[PafRecord], paf_file: &str) -> Result<Self, ParseErr> {
        ...
        let intervals: HashMap<u32, Vec<Interval<QueryMetadata>>> = records
            .par_iter()
            .filter_map(|record| {
            ...
            .fold(
                HashMap::new,
                |mut acc: HashMap<u32, Vec<Interval<QueryMetadata>>>, (target_id, interval)| {
            ...
            if let Some(tree) = self.trees.get(&current_target) {
                tree.query(current_start, current_end, |interval| {
                    ...
                    if metadata.query_id != current_target {
                        let todo_range =
                            (metadata.query_id, adjusted_query_start, adjusted_query_end);
...
fn parse_cigar_to_delta(cigar: &str) -> Result<Vec<CigarOp>, ParseErr> {
    ...
    for c in cigar.chars() {
        if c.is_ascii_digit() {
            ...
            let len = num_buf
                .parse::<i32>()
                .map_err(|_| ParseErr::InvalidCigarFormat)?;
...
mod tests {
    ...
    fn test_forward_projection_simple() {
        ...
        let (query_start, query_end, cigar, target_start, target_end) =
            project_target_range_through_alignment(target_range, record, &cigar_ops);
        assert_eq!(
